...
while not q.empty():
    ...
    if x_ >= 0 and x_ < X and y_ >= 0 and y_ < Y:
        q.put((cost + int(ls[y_][x_]), x_, y_, -dy, -dx, 0))

    x_ = x + dx
    y_ = y + dy

    if x_ >= 0 and x_ < X and y_ >= 0 and y_ < Y and s3 < 2:
...
def path(x, y, c):
    if x == 0 and y == 0:
        ...
        if len(dm) == 0:
            ...
            return [(x, y, cost)]
        return path(x - dm[0][0], y - dm[0][1], c - int(ls[y][x])) + [(x, y, cost)]
        
m = min(b[X - 1, Y -1].values())
